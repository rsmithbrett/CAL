...
// Tied to kMaxCards rather than an independent literal - this used to be a
// stale hardcoded 8, left over from when 8 was comfortably above the
// registration count, and it silently truncated any policy naming more than
// 8 cards (CheckIn.cpp's parseCardPolicy() drops the rest with only a log
// line, no error anywhere an operator would see). That was found live: a
// real policy with more than 8 entries lost everything past the eighth with
// no symptom on the device beyond "some of the cards I configured never show
// up." A policy can never usefully need more entries than there are
// registered cards to match them against (Cards::indexOf() is what a
// PolicyEntry is matched by, one-to-one), so tying the two together makes
// that class of silent truncation structurally impossible rather than
// something the next card-count bump has to remember to also apply here.
